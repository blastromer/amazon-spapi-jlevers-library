...
class SyncUpcCommand extends Command
{
    use ConsoleOutput, TimeStamp;

    protected $signature = 'amz-spapi-test:product-upc:sync';
    ...
    public function handle()
    {
        ...
        foreach ($items as $item) {
            ...
            $mongoProduct = $this->product->where('productId', (int) $item->getSku())->first();
            // $mongoProduct->getUpc();
            $title = $item->getTitle();
            if (!$mongoProduct->getTitle() == "" || !is_null($mongoProduct->getTitle())) {
                // dump($item->getTitle());
